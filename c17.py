#!/usr/bin/python
#
# The CBC padding oracle This is the best-known attack on modern block-cipher
# cryptography.
#
...
#
# What you're doing here. This pair of functions approximates AES-CBC encryption
...
from c11 import aescbc_encrypt, aescbc_decrypt, genaeskey
...
key = genaeskey()
crypt = b64decode(crypts[randrange(len(crypts))])
print repr(pkcs7pad(crypt))

def enc_oracle(clear, key):
    return aescbc_encrypt(pkcs7pad(clear), key)

def dec_oracle(iv, crypt):
    clear = aescbc_decrypt(iv, crypt, key)
    try:
        pkcs7unpad(clear)
        return True
    except ValueError:
...
# pseudocode:
#     xor (byte from last block, guessed letter, i) and submit as last byte
#     if no good, increment guess and guess again
#     if good, increment i, guess new letter + known letter, and repeat
def crack(iv, crypt):
    blocks = blockify(crypt, 16)
    print 'blocks length is: ' + str(len(blocks))
    print repr(crypt)
    print blocks
    current = blocks[-1]
    for i in range(256):
        ...
        trial = work + current
        print repr(trial)
        if dec_oracle(iv, trial):
            print 'Got a match!'
            #return repr(chr(i))
        else:
            print 'tried ' + repr(i) + ', no match'


if __name__ == '__main__':
    iv, encrypted = enc_oracle(crypt, key)
    print crack(iv, encrypted)
